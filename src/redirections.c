...
/*   Updated: 2025/03/19 11:57:01 by ral-haba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	heredoc(char *delimiter)
{
	...
	while(1)
	{
		...
		if (!line || ft_strcmp(line, delimiter) == 0)
			return(free(line), close(pipe_fd[1]), dup2(pipe_fd[0], STDIN_FILENO),
				close(pipe_fd[0]));
...
void	process_redirections(char **cmd)
{
	...
	while (cmd[i])
	{
		...
		if (fd != -1)
			dup2(fd, (!ft_strcmp(cmd[i - 1], "<")) ? STDIN_FILENO : STDOUT_FILENO);
		if (fd > 0)
			close(fd);
		i++;
	}
