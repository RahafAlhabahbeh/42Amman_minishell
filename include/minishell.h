...
int		handle_other_escapes(t_minishell *mini, size_t *i, char *buf,
			int *buf_i);
...
int		add_split_tokens(t_token **list, t_token **tail, char *str,
			t_token_type type);
int		process_token(t_minishell *mini, t_token *cur, t_token **list,
			t_token **tail);
int		extract_var_name(const char *str, int *pos, char *var_name,
			int *is_braced);
...
int		resolve_cmd_path_with_status(char *cmd, t_minishell *mini,
			char **path);
