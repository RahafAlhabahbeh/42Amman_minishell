...
int	handle_empty_command2(t_minishell *mini, t_cmd *cmd)
{
	...
	{
		if (cmd->input_file_name || cmd->output_file_name || cmd->redirections)
		{
			if (handle_empty_redirections(cmd) < 0)
				mini->exit_status = 1;
			else
				mini->exit_status = 0;
			return (1);
...
static void	handle_fork_result(t_minishell *mini, t_cmd *cmd, pid_t pid)
{
	char	**child_env;
...
void	execute_one_command(t_minishell *mini, char **envp)
{
	...
	cmd = mini->cmd;
	if (!cmd
		|| (cmd->in_type == HERE_DOC && handle_heredoc(mini, cmd) < 0)
