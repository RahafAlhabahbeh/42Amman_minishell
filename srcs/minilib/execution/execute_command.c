...
//     if (is_builtin(cmd->argv[0]))
//     {
//         if (should_run_builtin_in_parent(cmd, i, mini->pipex_count))
//         {
//             handle_redirections(cmd, prev_fd, pipe_fds, i == mini->pipex_count);
//             execute_builtin(mini, i);
//             return (i < mini->pipex_count) ? pipe_fds[0] : -1;
//         }
//         else if (is_str_in_set(cmd->argv[0], (char *[]){"export", "unset", "cd", "exit", NULL}) &&
//                  is_redirection_present(cmd))
//         {
//             // Create empty file(s) if redirection exists, without executing the command
//             handle_redirections(cmd, prev_fd, pipe_fds, i == mini->pipex_count);
//             return (i < mini->pipex_count) ? pipe_fds[0] : -1;
//         }
//     }
